...
#so fisrt they have us essentially just print the image (I did 2) we could prolly change this up in the future :)
...
test_images <- lapply(test_path, function(path) {
  image_read(path) %>%  #read image from file
  image_scale("x400")   #resizes it
})
...
for (i in seq_along(cols_vec)) {
  fig_name <- paste0(names(cols_vec)[i], "_square")  # create object name like "green_square"
  assign(
    fig_name,  # set name
  ...
  plot.new()  # creats a new blank space
  rect(0, 0, 1, 1, col = cols_vec[i], border = cols_vec[i])  # build rectangle, fills it with color
...
#This matches up with the red,blue,green..colors we did earlier so we need to add to this as we add colors
...
results_list <- list() #store into list
for (i in seq_along(test_images)) {
  test <- test_images[[i]]
  
#map the image with the new colors
  test_mapped <- image_map(image = test, map = simple_cols)
  
# Animates oringinal and mapped version I tried to get rid of the flickering but it didn't work idk how
...
test_col_freq <- test_mapped %>% #starts test_mapped dataframe
  count_colors() %>%
  left_join(
